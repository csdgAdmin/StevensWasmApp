...
public class LoginService : ILoginService
{
    ...
    /// </summary>
    /// <param name="endPointUri">The request endpoint for token retrieval.</param>
    ...
    public async Task<string> GetBearer(string endPointUri, UserLoginDto userLoginDto)
    {
        string fullAddress = string.Join("api/", this._httpClient.BaseAddress, endPointUri);
        try
        {
            /* Await the response read activity to increase
             * likelyhood of blocking until data is formed & retrieved
             */
